...
public class CombinatorParser implements IAqlParser {
	...
	private static String[] ops2 = { "{", "}" };
	private static final Terminals RESERVED = Terminals.caseSensitive(Util.union(ops, ops2),
			Util.union(xxx(), Util.union(res, opts)));

	private static final Parser<Void> IGNORED = Parsers.or(Scanners.lineComment("#"), Scanners.WHITESPACES).skipMany();
	...
	private static void tyExp() {
		Parser<TyExp> var = ident.map(TyExpVar::new), sql = token("sql").map(x -> new TyExpSql()),
				...
				ret = Parsers.or(sch, empty, sql, tyExpRaw(), excel, var, rdf, sql2, parens(ty_ref));
	...
	private static void schExp() {
		...
		Parser<SchExp> var = ident.map(SchExpVar::new),
				...
				ms_sql3 = Parsers.tuple(token("ms_error"), ident, ident.followedBy(token(":")), ty_ref.lazy())
						.map(x -> new SchExpMsError(x.b, x.c, x.d)),
				ms_sql4 = Parsers.tuple(token("ms_error_shallow"), ident.followedBy(token(":")), ty_ref.lazy())
						.map(x -> new SchExpMsErrorShallow(x.b, x.c)),
				ms_sql5 = Parsers
						.tuple(token("from_ms_catalog"), inst_ref.lazy(),
	...
	private static void pragmaExp() {
		...
		Parser<PragmaExp> var = ident.map(PragmaExpVar::new),
				...
				bitsy = Parsers.tuple(token("spawn_bitsy"), options.between(token("{"), token("}")).optional())
						.map(x -> new PragmaExpBitsy(x.b == null ? Collections.emptyList() : x.b)),

				tp = Parsers.tuple(token("exec_tinkerpop"), p).map(x -> new PragmaExpTinkerpop(x.b.a, x.b.b)),
				tpe = Parsers
						.tuple(Parsers.tuple(token("export_tinkerpop_instance"), inst_ref.lazy()),
								options.between(token("{"), token("}")).optional())
						.map(x -> new PragmaExpTinkerpopInstExport(x.a.b, x.b == null ? Collections.emptyList() : x.b)),
				...
				check = Parsers
						.tuple(token("check"), edsExp(), inst_ref.lazy(),
								options.between(token("{"), token("}")).optional())
						.map(x -> new PragmaExpCheck(x.c, x.b, x.d == null ? Collections.emptyList() : x.d)),
				...
				ret = Parsers.or(jdbcQuery, rdfInst2, check, check2, jsonInst, csvInst, cons, bitsy, csvTrans, var, sql,
						js, proc, jdbcInst, jdbcTrans, match, excelInst, rdfInst, tp, tpe, parens(pragma_ref));
	...
	private static void instExp() {
		...
		Parser<InstExp<?, ?, ?, ?>> var = ident.map(InstExpVar::new),
				...
				ms_sql3 = Parsers.tuple(token("ms_error"), ident, ty_ref.lazy(), ident)
						.map(x -> new InstExpMsError(x.b, x.d, x.c)),
	...
	public static Parser<Links<String, String>> links() {

		Parser<List<catdata.Pair<String, Quad<String, String, String, String>>>> enLinks = Parsers
				.tuple(ident.followedBy(token(":")), ident.followedBy(token(".")), ident.followedBy(token("->")),
						ident.followedBy(token(".")), ident)
				.map(z -> new catdata.Pair<>(z.a, new Quad<>(z.b, z.c, z.d, z.e))).many();

		Parser<Pair<RawTerm, RawTerm>> www = Parsers.tuple(term().followedBy(token("=")), term());

		Parser<List<catdata.Pair<String, Quad<String, catdata.Pair<String, String>, RawTerm, RawTerm>>>> colLinks = Parsers
				.tuple(ident.followedBy(token(":")).followedBy(token("forall")), ident.followedBy(token(":")),
						ident.followedBy(token(".")), ident.followedBy(token(",")), www)
				.map(z -> new catdata.Pair<String, Quad<String, catdata.Pair<String, String>, RawTerm, RawTerm>>(z.a,
						new Quad<>(z.b, new catdata.Pair<>(z.c, z.d), z.e.a, z.e.b)))
				.many();

		Parser<List<catdata.Pair<String, EdExpRaw>>> rowLinks = Parsers.tuple(ident.followedBy(token(":")), edExpRaw())
				.map(x -> new catdata.Pair<String, EdExpRaw>(x.a, x.b)).many();

		return Parsers
				.tuple(token("entity_isomorphisms"), enLinks.followedBy(token("equations")),
						colLinks.followedBy(token("constraints")), rowLinks)
				.map(x -> new Links<String, String>(Util.toMapSafely(x.b), Util.toMapSafely(x.c),
						Util.toMapSafely(x.d)));
	}
	...
	private static void mapExp() {
		Parser<MapExp> var = ident.map(MapExpVar::new),
				...
				pseudo = Parsers.tuple(token("getPseudo"), colim_ref.lazy()).map(x -> new MapExpPseudo(x.b)),
				...
				ret = Parsers.or(id, id2, mapExpRaw(), var, pseudo, pivot, colim, frompreix, toprefix, comp,
						parens(map_ref));
...
//      .map(x -> x == null ? new LinkedList() : x.b);
	...
	private static Parser<List<catdata.Pair<LocStr, catdata.Pair<List<catdata.Pair<LocStr, String>>, List<catdata.Pair<LocStr, String>>>>>> whee() {
		// Parser<catdata.Pair<LocStr, String>> q =
		// Parsers.tuple(locstr.followedBy(token(":")), ident).map(x-> new
		// catdata.Pair<>(x.a,x.b));
		Parser<List<catdata.Pair<LocStr, String>>> p1 = Parsers.tuple(token("foreign_keys"), env(ident, ":")).optional()
				.map(x -> x == null ? Collections.emptyList() : x.b);
		Parser<List<catdata.Pair<LocStr, String>>> p2 = Parsers.tuple(token("attributes"), env(ident, ":")).optional()
				.map(x -> x == null ? Collections.emptyList() : x.b);

		Parser<catdata.Pair<LocStr, catdata.Pair<List<catdata.Pair<LocStr, String>>, List<catdata.Pair<LocStr, String>>>>> p0 = Parsers
				.tuple(token("entity"), locstr, p1, p2).map(x -> new catdata.Pair<>(x.b, new catdata.Pair<>(x.c, x.d)));
	...
	private static Parser<SchExpRaw> schExpRaw() {
		...
		Parser<Tuple5<List<SchExp>, List<catdata.Pair<LocStr, catdata.Pair<List<catdata.Pair<LocStr, String>>, List<catdata.Pair<LocStr, String>>>>>, List<LocStr>, List<catdata.Pair<LocStr, catdata.Pair<String, String>>>, List<catdata.Pair<Integer, catdata.Pair<List<String>, List<String>>>>>> pa = Parsers
				.tuple(imports(sch_ref.lazy()), whee(), entities.optional(), fks0.optional(), p_eqs0.optional());
		...
		Parser<SchExpRaw> ret = Parsers.tuple(l, pa, pb, token("}"))
				.map(x -> new SchExpRaw(x.a.c, x.b.a, Util.newIfNull(x.b.b), Util.newIfNull(x.b.c),
						Util.newIfNull(x.b.d), Util.newIfNull(x.b.e), Util.newIfNull(x.c.a), Util.newIfNull(x.c.b),
						x.c.c));

		return ret;
	}

	private static Parser<ColimSchExpPseudo> colimSchExpPseudo() {
		Parser<catdata.Pair<Integer, catdata.Pair<String, Quad<String, String, String, String>>>> q = Parsers
				.tuple(Parsers.INDEX, Parsers
						.tuple(ident.followedBy(token(":")),
								Parsers.tuple(ident.followedBy(token(".")), ident, token("->"),
										ident.followedBy(token(".")), ident).map(x -> new Quad<>(x.a, x.b, x.d, x.e)))

						.map(z -> new catdata.Pair<>(z.a, z.b)))
				.map(x -> new catdata.Pair<>(x.a, x.b));

		var entities = Parsers.tuple(token("entity_isomorphisms"), q.many()).map(x -> x.b);

		Parser<catdata.Pair<Integer, Quad<String, catdata.Pair<String, String>, RawTerm, RawTerm>>> o_eq = Parsers
				.tuple(Parsers.INDEX, Parsers
						.tuple(token("forall"), ident,
								Parsers.tuple(token(":"),
										Parsers.tuple(ident, token("."), ident).map(x -> new catdata.Pair<>(x.a, x.c)))
										.followedBy(token(",")),
								term().followedBy(token("=")), term())
		...
		/*
		 * Parser<catdata.Pair<Integer, Quad<String, String, RawTerm, RawTerm>>>
		 * o_eq_from_p_eq = p_eq .map(x -> new catdata.Pair<>(x.first, new Quad<>("_x",
		 * null, RawTerm.fold(x.second.first, "_x"), RawTerm.fold(x.second.second,
		 * "_x"))));
		 * 
		 * Parser<catdata.Pair<Integer, Quad<String, String, RawTerm, RawTerm>>> o_eq =
		 * Parsers.or(o_eq_old, o_eq_from_p_eq);
		 */

		Parser<Pair<Token, List<catdata.Pair<Integer, Quad<String, catdata.Pair<String, String>, RawTerm, RawTerm>>>>> o_eqs = Parsers
				.tuple(token("equations"), o_eq.many());
		Parser<List<catdata.Pair<Integer, Quad<String, catdata.Pair<String, String>, RawTerm, RawTerm>>>> o_eqs0 = o_eqs
				.map(x -> x.b);

		var pa = Parsers.tuple(entities.optional(), o_eqs0.optional(), options);

		Parser<Tuple4<Token, List<LocStr>, Token, TyExp>> l = Parsers.tuple(token("pseudo_quotient"),
				locstr.sepBy(token("+")), token(":"), ty_ref.lazy());

		var ppp = Parsers.tuple(token("{"), pa, token("}")).map(x -> x.b);

		Parser<ColimSchExpPseudo> ret = Parsers.tuple(l, ppp).map(x -> {
			TyExp ty = x.a.d;
			List<LocStr> nodes = x.a.b;
			if (x.b == null)
				Util.anomaly();

			return new ColimSchExpPseudo(ty, nodes, Util.newIfNull(x.b.a), Util.newIfNull(x.b.b), x.b.c);
	...
	private static Parser<EdsExpRaw> edsExpRaw() {
	...
	private static Parser<EdsExp> edsExp() {
		Parser<EdsExp> var = ident.map(EdsExpVar::new),
				...
				sql = Parsers.tuple(token("sql"), sch_ref.lazy()).map(x -> new EdsExpSql(x.b)),
				sqlNull = Parsers.tuple(token("sqlNull"), ty_ref.lazy()).map(x -> new EdsExpSqlNull(x.b)),
				tp = (token("tinkerpop")).map(x -> new EdsExpTinkerpop()),
				edsSigma = Parsers.tuple(token("sigma"), map_ref.lazy(), eds_ref.lazy())
						.map(x -> new EdsExpSigma(x.b, x.c)),
				ms_sql5 = Parsers
						.tuple(token("from_ms_catalog"), inst_ref.lazy().followedBy(token(":")), sch_ref.lazy(),
								options.between(token("{"), token("}")).optional())
	...
	private static void queryExp() {
		...
		Parser<QueryExp> var = ident.map(QueryExpVar::new),
				...
				front = Parsers
						.tuple(token("back"), eds_ref.lazy(), ident, options.between(token("{"), token("}")).optional())
						.map(x -> new QueryExpFront(x.b, x.c, true, x.d == null ? Collections.emptyList() : x.d)),
				back = Parsers
						.tuple(token("front"), eds_ref.lazy(), ident,
								options.between(token("{"), token("}")).optional())
						.map(x -> new QueryExpFront(x.b, x.c, false, x.d == null ? Collections.emptyList() : x.d)),
	...
	private static void colimSchExp() {
		...
		Parser<ColimSchExp> simpl = Parsers
				.tuple(token("simplify"), colim_ref.lazy(), options.between(token("{"), token("}")).optional())
				.map(x -> new ColimSchExpSimplify(x.b, x.c == null ? Collections.emptyList() : x.c));

		Parser<ColimSchExp> retX = Parsers.or(ret, ret2, ret3, colimExpModify(), colimSchExpPseudo(),
				colimSchExpQuotient(), simpl, parens(colim_ref));
	...
	private static Parser<InstExpTinkerpop> instExpTinkerpop() {
		Parser<InstExpTinkerpop> ret = Parsers
				.tuple(token("import_tinkerpop_all"), options.between(token("{"), token("}")).optional())
				.map(x -> new InstExpTinkerpop(Util.newIfNull(x.b)));
		return ret;
	}
	...
	private static Parser<Program<Exp<?>>> program(String s) {
		...
		apgSchExp();
		apgMapExp();
		Parser<List<String>> is = Parsers.tuple(token("imports"), ident.many()).map(x -> x.b).optional();
		...
		return Parsers.tuple(is, options, p.many()).map(x -> new Program((x.c), s, x.b,
				q -> ((Exp) q).kind().toString(), x.a == null ? Collections.emptyList() : x.a));
	}
	...
	public static Links<String, String> parseLinks(String s) {
