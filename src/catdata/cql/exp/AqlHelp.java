...
public class AqlHelp {
  ...
  private static String getProverText(ProverName p) {
    ...
    // Graceful fallback: a prover without a hand-written description must not crash
    // the whole documentation build. The gap stays visible on the generated page.
    return "TODO: this prover is not yet documented.";
  }
  ...
  static String getOptionText2(AqlOption op) {
    ...
    // Graceful fallback: an option without a hand-written description must not crash
    // the whole documentation build. The gap stays visible on the generated page.
    return "TODO: this option is not yet documented.";
  }
  ...
  public String report() {
    ...
    Map<Kind, Set<Pair<AqlTyping, Exp<?>>>> exps =
        new TypingInversion().covisit(Unit.unit, x -> new AqlTyping());
